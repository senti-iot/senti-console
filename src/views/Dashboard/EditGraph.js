...
import { Paper, Dialog, AppBar, IconButton, Hidden, withStyles, Toolbar, Drawer, Fade } from '@material-ui/core';
	...
	typeChildren = (g) => {
		...
		if (g)
			switch (g.type) {
				...
				case 0:
					// return  <Paper key={g.id} data-grid={{ ...g.grid, x: 2, y: 5 }}> */}
					return <Fade in={true} timeout={1000} key={g.id} data-grid={{ ...g.grid, x: 2, y: 5 }}>
						<Paper key={g.id}>
							<DoubleChart
								create
								title={g.name}
